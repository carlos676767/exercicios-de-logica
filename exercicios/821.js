function high(sentence) {
  const alphabet = Array.from(Array(26).keys()).map((index) => String.fromCharCode(index + 97) );

  const words = sentence.split(" ").map((word) => word.split(""));

  const wordScores = words.map((letters) => letters.map((letter) => alphabet.indexOf(letter) + 1) 
